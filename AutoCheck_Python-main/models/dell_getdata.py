...
class Dell:
    def __init__(self, switch):
        self.switch = switch
        self.rawdata = list()
        self.cmd = [' term length 0', 'show system', 'show interface status', 'show interfaces | grep "protocol is|Input|Output"',
                    'show interfaces | grep "protocol is|CRC"', 'show process cpu', 'show ip route', 'show system brief','show process mem', 
                    'show system stack-unit 1', 'show ntp associations', 'show logging', 'exit']
    ...
    def get_telnet(self):
        ...
        for i in self.cmd:
            cmd += i+'\n'
        if vendor != 'dell' or protocol != 'telnet':
        ...
        for i in self.cmd:
            try:
                ...
                ssh.connect(hostname=ip, port=ports,
                            username=uid, password=passwd, banner_timeout=200)
    ...
    def get_tacacs(self):
        ...
        data = str()
        data += 'ipaddr: '+ip+'\n'
        if proto != 'tacacs' or vendor != 'dell':
            print('Not supported switch %s or vendor %s' % (vendor, proto))
            exit()
