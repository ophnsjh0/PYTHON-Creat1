...
class CiscoNX:
    def __init__(self, switch):
        self.switch = switch
        self.rawdata = list()
        self.cmd = [' term length 0', 'show version', 'show logging', 'show interface status', 'show interface counters',
                    'show interface count error', 'show module', 'show system resource', 'show environment temperature', 
                    'show environment power', 'show environment fan', 'show interfaces transceiver detail', 'show spanning-tree', 
                    'show ntp peer-status', 'show ip route', 'exit']
    ...
    def get_telnet(self):
        ...
        for i in self.cmd:
            cmd += i+'\n'
        if vendor != 'cisconx' or protocol != 'telnet':
    ...
    def get_ssh(self):
        ...
        data = str()
        data += 'ipaddr: '+ip+'\n'
        if proto != 'ssh' or vendor != 'cisconx':
    ...
    def get_tacacs(self):
        ...
        data = str()
        data += 'ipaddr: '+ip+'\n'
        if proto != 'tacacs' or vendor != 'cisconx':
            print('Not supported switch %s or vendor %s' % (vendor, proto))
            exit()
