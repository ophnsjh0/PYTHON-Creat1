...
class Citrix:
    def __init__(self, switch):
        self.switch = switch
        self.rawdata = list()
        self.cmd = ['show ns event', 'show interface', 'show interface | grep -E "Interface|RX|TX"', 'show route',
                    'stat ns', 'stat system cpu', 'stat system -detail | grep Temp', 'stat system -detail | grep Power',
                    'stat system -detail | grep Fan','show lb vserver -summary -fullValues', 'show service -summary -fullValues', 
                    'show acl -summary', 'show ntp status', 'show gslb vserver -summary -fullValues', ' show gslb service -summary -fullValues', 
                    'show hardware', 'show hostname','exit']
    ...
    def get_telnet(self):
        ...
        for i in self.cmd:
            cmd += i+'\n'
        if vendor != 'citrix' or protocol != 'telnet':
    ...
    def get_tacacs(self):
        ...
        data = str()
        data += 'ipaddr: '+ip+'\n'
        if proto != 'tacacs' or vendor != 'citrix':
            print('Not supported switch %s or vendor %s' % (vendor, proto))
            exit()
        for i in self.cmd:
            try:
